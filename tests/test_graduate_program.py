...
def test_query_single_graduate_program(client, institution, graduate_program):
    endpoint = f"/GraduateProgramRest/Query?institution_id={institution[0]["institution_id"]}"
...
def test_query_graduate_programs(client, institution, graduate_program, another_graduate_program):
    endpoint = f"/GraduateProgramRest/Query?institution_id={institution[0]["institution_id"]}"
    response = client.get(endpoint)
...
def test_update_visible_graduate_program(client, institution, graduate_program):
    endpoint = f"/GraduateProgramRest/Update?graduate_program_id={graduate_program[0]["graduate_program_id"]}"
...
def test_return_visible_graduate_program(client, institution, graduate_program):
...
def test_fix_graduate_program(client, institution, graduate_program):
    ...
    graduate_program[0].update(
        {"qtd_colaborador": 0, "qtd_discente": 0, "qtd_permanente": 0}
    )
    assert graduate_program[0] in response.json


def test_delete_graduate_program(client, institution, graduate_program):
    endpoint = f"/GraduateProgramRest/Delete?graduate_program_id={graduate_program[0]['graduate_program_id']}"
    response = client.delete(endpoint)
    assert response.status_code == HTTPStatus.NO_CONTENT


def test_count_graduate_program(client, institution, graduate_program):
    endpoint = f"/GraduateProgramRest/Query/Count?institution_id={institution[0]["institution_id"]}"
    response = client.get(endpoint)
    assert response.json == 1


def test_count_graduate_program_no_filter(client, institution, graduate_program):
